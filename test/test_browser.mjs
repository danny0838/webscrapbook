...
async function buildAndRunTests({driver, extensionUrl, grep, reporter}) {
  ...
  function renderSuiteTree(suiteNode) {
    suiteNode.hooks.forEach((hookNode) => {
      const {id, type, title, timeout, slow} = hookNode;
      const {promise, resolve, reject} = Promise.withResolvers();
      testPromises.set(id, {promise, resolve, reject});
      globalThis[type](title, async function () {
        this.timeout(timeout !== 0 ? timeout + 5000 : timeout);
        this.slow(slow !== 0 ? slow + 2500 : slow);
    ...
    suiteNode.tests.forEach((test) => {
      const {id, title, timeout, slow} = test;
      const {promise, resolve, reject} = Promise.withResolvers();
      testPromises.set(id, {promise, resolve, reject});

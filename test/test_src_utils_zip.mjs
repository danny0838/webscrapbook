...
import {Zip, isCompressible} from "./shared/utils/zip.mjs";
...
describe('utils/zip.mjs', function () {
  ...
  describe('isCompressible()', function () {
    it('should return true for text types', async function () {
      assert.isTrue(isCompressible('text/plain'));
      assert.isTrue(isCompressible('text/html'));
      assert.isTrue(isCompressible('text/css'));
      assert.isTrue(isCompressible('text/javascript'));
      assert.isTrue(isCompressible('text/markdown'));
    });

    it('should return true for text suffixes', async function () {
      assert.isTrue(isCompressible('application/xhtml+xml'));
      assert.isTrue(isCompressible('image/svg+xml'));
      assert.isTrue(isCompressible('application/ld+json'));
    });

    it('should return true for special types', async function () {
      assert.isTrue(isCompressible('application/javascript'));
      assert.isTrue(isCompressible('application/ecmascript'));
      assert.isTrue(isCompressible('application/x-ecmascript'));
    ...
    it('should return false for falsy value', async function () {
      assert.isFalse(isCompressible());
      assert.isFalse(isCompressible(null));
      assert.isFalse(isCompressible(false));
      assert.isFalse(isCompressible(''));
    });

    it('should return false for binary types', async function () {
      assert.isFalse(isCompressible('image/jpeg'));
      assert.isFalse(isCompressible('application/octet-stream'));
      assert.isFalse(isCompressible('application/ogg'));
      assert.isFalse(isCompressible('application/pdf'));
      assert.isFalse(isCompressible('application/zip'));
      assert.isFalse(isCompressible('application/x-rar-compressed'));
      assert.isFalse(isCompressible('application/x-gzip'));
      assert.isFalse(isCompressible('application/html+zip'));
      assert.isFalse(isCompressible('application/x-maff'));
    });

    it('should return false for binary suffixes', async function () {
      assert.isFalse(isCompressible('application/epub+zip'));
    });
  });
});

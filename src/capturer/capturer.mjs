...
class Capturer extends BaseCapturer {
   ...
   */
  captureInfo = new MapWithDefault(() => ({
    useDiskCache: false,
  ...
  async invoke(method, args, details = {}) {
    ...
    if (Number.isInteger(tabId)) {
      ...
      return utils.invokeMethod(this, method, args);
    }
  }
   ...
   */
  getUniqueFilename(timeId, filename, options) {
   ...
   */
  async getAvailableSaveFilename(params) {
  ...
  async fetch(params) {
    ...
    const fetch = this.fetch = async (params) => {
      ...
      const {fetchMap} = this.captureInfo.get(timeId);
   ...
   */
  async addItemToServer({item, parentId, index}) {
    await server.init();
    ...
    this.log(`Updating server index for item "${item.id}"...`);
    ...
    try {
      ...
      console.warn(ex);
      this.warn(utils.lang("ErrorFileDownloadError", [icon, ex.message]));
    }
    ...
    await book.transaction({
      ...
      callback: async (book, {updated}) => {
        ...
        if (parentId && !book.meta[parentId] && !book.isSpecialItem(parentId)) {
          this.warn(`Specified parent ID "${parentId}" is invalid. Append to root instead.`);
          parentId = 'root';
        }
  ...
  async runTasks({
    ...
    for (const task of tasks) {
      ...
      let result;
      try {
        this.addItemToServer.added = false;

        if (["resave", "internalize"].includes(mode)) {
          result = await this.resaveTab({
            tabId, frameId,
            options,
          ...
          // merge capture
          result = await this.mergeCapture({
            tabId, frameId,
            url, refUrl,
            mode,
            settings: {fullPage, title, favIconUrl},
          ...
          // capture general
          result = await this.captureGeneral({
            tabId, frameId,
            url, refUrl,
            mode,
            settings: {fullPage, title, favIconUrl},
  ...
  async captureGeneral({
        ...
        try {
          ...
          const _docUrl = await (async () => {
            if (doc) {
              ...
              const redirectInfo = await this.resolveRedirects({
                url,
                refUrl,
                settings: {
          ...
          this.warn(`Ignored invalid capture.helpers: ${ex.message}`);
        }
      } else {
        options["capture.helpersEnabled"] = false;
      }
    ...
    if (doc) {
      ...
      // capture headless
      response = await this.captureRemote({
        url, refUrl,
        mode, settings, options,
      });
    } else {
    ...
    if (!captureOnly) {
      ...
      this.captureInfo.delete(timeId);
      await this.clearFileCache({timeId});
    }
  ...
  async captureRemote({
    ...
    switch (mode) {
      case "tab": {
        captureFunc = this.captureRemoteTab;
        break;
      }
      case "bookmark": {
        captureFunc = this.captureBookmark;
        break;
      }
      case "source":
      default: {
        captureMode = "source";
        captureFunc = this.captureUrl;
        break;
      }
    }
  ...
  async captureUrl(params) {
    ...
    if (downLink && !downLinkExtra) {
      ...
      if (this.downLinkUrlFilter(sourceUrl, options)) {
        return null;
      }
      ...
      if (downLinkFileValid && options["capture.downLink.file.mode"] === "url") {
        ...
        if (!this.downLinkFileExtFilter(ext, options)) {
          downLinkFileValid = false;
        }
      }

      // apply in-depth URL filter
      if (downLinkDocValid && !this.downLinkDocUrlFilter(sourceUrl, options)) {
    ...
    if (downLink) {
      if (downLinkDoc && doc) {
        ...
        if (downLinkDocValid || downLinkExtra) {
          const {linkedPages} = this.captureInfo.get(timeId);
      ...
      if (!downLinkExtra && downLinkFileValid && options["capture.downLink.file.mode"] === "header") {
        if (!redirectInfo.error) {
          ...
          if (!(this.downLinkFileMimeFilter(mime, options) || this.downLinkFileExtFilter(ext, options))) {
            downLinkFileValid = false;
          }
        } else {
          downLinkFileValid = false;
      ...
      if (downLinkExtra) {
        this.warn(`Skipped invalid extra URL: "${sourceUrl}"`);
      }
    ...
    if (doc) {
      if (downLinkPage && options["capture.downLink.doc.mode"] === "tab") {
        const response = await this.captureRemoteTab({
          url,
          refUrl,
          settings,
          options,
...
Bookmark for <a href="${utils.escapeHtml(sourceUrl)}">${utils.escapeHtml(sourceUrl, false)}</a>
  ...
  async captureFile(params) {
    ...
    let response = await this.downloadFile({
      url: sourceUrl,
      refUrl,
      refPolicy,
      settings,
    ...
    if (settings.type === 'site') {
      return await this.saveMainDocument({
        sourceUrl,
...
Redirecting to file <a href="${utils.escapeHtml(response.url)}">${utils.escapeHtml(response.filename, false)}</a>
  ...
  async resaveTab({
    ...
    this.log(`Saving (document) ${source} ...`);
    ...
    await book.transaction({
      ...
      callback: async (book, {updated}) => {
        ...
        isDebug && console.debug("(main) send", source, message);
        const response = await this.invoke("retrieveDocumentContent", message, {tabId, frameId});
        isDebug && console.debug("(main) response", source, response);
        ...
        if (internalize) {
          ...
          for (const [fileUrl, data] of Object.entries(response)) {
            const fetchResource = async (url) => {
              const fullUrl = utils.normalizeUrl(this.resolveRelativeUrl(url, fileUrl));
              ...
              try {
                ...
                console.error(ex);
                this.warn(`Unable to internalize resource "${utils.crop(url, 256)}": ${ex.message}`);
              }
            };
        ...
        for (const [fileUrl, data] of Object.entries(response)) {
          try {
            ...
            let {blob} = data;
            blob = await this.loadBlobCache(blob);
        ...
        for (const [url, file] of resourceMap.entries()) {
          ...
          this.log(`Internalized resource: ${utils.crop(target, 256)}`);
        }
        ...
        if (item) {
          ...
          if (!book.config.no_tree) {
            this.warn(utils.lang("ErrorSaveUnknownItem"));
          }
        }
      },
    });
  ...
  async recapture({
    ...
    this.log(`Preparing a re-capture for item "${itemId}" of book "${bookId}"...`);
    ...
    await book.transaction({
      ...
      callback: async (book, {updated}) => {
        ...
        try {
          ...
          console.warn(ex);
          this.warn(utils.lang("ErrorFileDownloadError", [item.icon, ex.message]));
        }
        ...
        migrateAnnotations: {
          ...
          try {
            ...
            console.error(ex);
            this.warn(`Failed to read old page. Skipped migrating annotations.`);
          ...
          for (const error of errors.reverse()) {
            this.warn(`Failed to migrate annotation: ${error}`);
          }
        ...
        // update item meta
        this.log(`Updating server index for item "${itemId}"...`);
        await server.request({
          query: {
            a: 'query',
            lock: '',
        ...
        if (oldIndex) {
          ...
          if (options["capture.backupForRecapture"]) {
            ...
            this.log(`Moving old data files "${index}" to backup directory "`, anchor, `"...`);
            ...
            this.log(`Deleting old data files "${index}"...`);
            try {
              await server.request({
                url: target + `?a=delete`,
                method: "POST",
  ...
  async mergeCapture({
    ...
    this.log(`Preparing a merge capture for item "${itemId}" of book "${bookId}"...`);
    ...
    await book.transaction({
      ...
      callback: async (book, {updated}) => {
        ...
        const info = this.captureInfo.get(timeId);
        ...
        await this.loadSiteMap({sitemap, info, timeId, indexUrl});
   ...
   * @property {boolean} [isDuplicate] - True if the provided resource has been
   *   registered.
   */
  ...
  async registerDocument(params) {
    ...
    const registerDocument = this.registerDocument = async (params) => {
      ...
      const {filenameMap, files} = this.captureInfo.get(timeId);
      ...
      if (role || (isMainPage && isMainFrame)) {
        const token = this.getRegisterToken(sourceUrlMain, role);
        if (!token) {
        ...
        if (documentName && ((isMainPage && isMainFrame) || options["capture.frameRename"])) {
          ...
          // see this.getUniqueFilename for filename limitation
          ...
          documentFileName = this.getUniqueFilename(settings.timeId, documentFileName, options);
        }
  ...
  async registerFile(params) {
    ...
    const registerFile = this.registerFile = async (params) => {
      ...
      let response;
      if (role) {
        const token = this.getRegisterToken(fetchResponse.url, role);
        if (!token) {
        ...
        filename = this.getUniqueFilename(settings.timeId, filename, options);
  ...
  downLinkFileExtFilter(ext, options) {
    let filterText;
    let filters;
    const fn = this.downLinkFileExtFilter = (ext, options) => {
      ...
      if (filterText !== options["capture.downLink.file.extFilter"]) {
        ...
        try {
          ...
          this.warn(`Ignored invalid capture.downLink.file.extFilter: ${ex.message}`);
          filters = [];
        }
      }
  ...
  downLinkFileMimeFilter(mime, options) {
    let filterText;
    let filters;
    const fn = this.downLinkFileMimeFilter = (mime, options) => {
      // use the cache if the filter is not changed
      if (filterText !== options["capture.downLink.file.extFilter"]) {
        filterText = options["capture.downLink.file.extFilter"];
        try {
  ...
  downLinkDocUrlFilter(url, options) {
    let filterText;
    let filters;
    const fn = this.downLinkDocUrlFilter = (url, options) => {
      // use the cache if the filter is not changed
      if (filterText !== options["capture.downLink.doc.urlFilter"]) {
        ...
        try {
          ...
          this.warn(`Ignored invalid capture.downLink.doc.urlFilter: ${ex.message}`);
          filters = [];
        }
      }
  ...
  downLinkUrlFilter(url, options) {
    let filterText;
    let filters;
    const fn = this.downLinkUrlFilter = (url, options) => {
      // use the cache if the filter is not changed
      if (filterText !== options["capture.downLink.urlFilter"]) {
        ...
        try {
          ...
          this.warn(`Ignored invalid capture.downLink.urlFilter: ${ex.message}`);
          filters = [];
        }
      }
  ...
  async saveDocument(params) {
    ...
    const downloadBlob = async () => {
      const blob = await this.loadBlobCache(data.blob);
    ...
    return this.saveMainDocument({data, sourceUrl, documentFileName, settings, options});
  }
...
Redirecting to <a href="${utils.escapeHtml(target)}">${utils.escapeHtml(target, false)}</a>
    ...
    this.captureInfo.get(timeId).indexPages.add(documentFileName);

    // handle in-depth capture
    if (itemType === 'site') {
      const sitemapPath = options["capture.saveAs"] === 'maff' ? `${timeId}/index.json` : 'index.json';

      await this.captureLinkedPages({settings, options});

      this.log('Rebuilding links...');
      await this.rebuildLinks({timeId, options});
      await this.dumpSiteMap({timeId, path: sitemapPath});
    }
    ...
    switch (saveAs) {
      ...
      case "zip": {
        ...
        const zip = await this.loadFileCacheAsZip({timeId, options});
...
`;
      ...
      default: {
        ...
        getTargetDirName: {
          ...
          const newFilename = await this.invoke("getAvailableSaveFilename", [{
  ...
  async downloadBlob(params) {
    ...
    const downloadBlob = this.downloadBlob = async (params) => {
   ...
   */
  async saveBlobNaturally(params) {
  ...
  async saveUrl(params) {
    ...
    return await new Promise((resolve, reject) => {
      this.downloadHooks.set(downloadId, {
        timeId,
        src: sourceUrl,
        autoErase,
        onComplete: resolve,
   ...
   */
  async captureLinkedPages(params) {
    ...
    for (const [sourceUrl, {url, refUrl, depth}] of linkedPages) {
      ...
      this.log(`Capturing linked page (${depth}) ${sourceUrl} ...`);
      ...
      const response = await this.captureUrl({
        ...
        console.error(ex);
        this.error(`Subpage fatal error (${url}): ${ex.message}`);
        return {url: this.getErrorUrl(url, options), error: {message: ex.message}};
      });

      // add pages with depth 0 to indexPages
      if (depth === 0) {
        this.captureInfo.get(timeId).indexPages.add(response.filename);
      }
   ...
   */
  async dumpSiteMap({timeId, path}) {
    const version = 3;
    const {
      files, filenameMap,
      initialVersion, indexPages, redirects,
    } = this.captureInfo.get(timeId);
    ...
    for (let [filename, {path, url, role, token}] of files) {
      if (!token) {
        const t = this.getRegisterToken(url, role);
  ...
  async loadSiteMap(...args) {
    ...
    const fn = this.loadSiteMap = async ({sitemap, info, timeId, indexUrl}) => {
      ...
      switch (sitemap.version) {
        case 1: {
          for (const {path, url, role, primary} of sitemap.files) {
            ...
            if (primary) {
              const token = this.getRegisterToken(url, role);
              if (!token) {

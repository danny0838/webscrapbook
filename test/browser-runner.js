...
async function launchDriver({profileDirectory, browserName, exePath, headless, incognito, manifest}) {
  ...
  if (browserName === "firefox") {
    ...
    const version = (() => {
      ...
      const output = execFileSync(browserPath, ["--version"], {encoding: "utf8"});
      const match = output.match(/\b(\d+)\./);
      if (!match) {
        throw new Error(`Unable to detect Firefox version: ${output}`);
      }
      return Number(match[1]);
    })();

    const service = new firefox.ServiceBuilder();
    if (version >= 138) { service.addArguments("--allow-system-access"); }
